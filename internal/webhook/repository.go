...
const deliveryColumns = `id,subscription_id,tenant_id,event_id,event_subject,payload,status,attempt_count,next_attempt_at,last_attempt_at,response_status,response_body,error_message,delivered_at,version,created_at,updated_at,created_by,updated_by`
...
func (r *Repository) CreateSubscription(ctx context.Context, value Subscription) (Subscription, error) {
	...
	return r.GetSubscription(ctx, value.TenantID, value.ID)
}
...
func (r *Repository) UpdateSubscription(ctx context.Context, value Subscription, expectedVersion int64, actor string) (Subscription, error) {
	now := r.now()
	query := r.db.Rebind(`UPDATE webhook_subscriptions SET name=?,endpoint_url=?,subject_filter=?,status=?,timeout_ms=?,max_attempts=?,retry_initial_seconds=?,version=version+1,updated_at=?,updated_by=? WHERE id=? AND tenant_id=? AND version=? AND deleted_at IS NULL`)
	result, err := r.db.ExecContext(ctx, query, value.Name, value.EndpointURL, value.SubjectFilter, value.Status, value.TimeoutMS, value.MaxAttempts, value.RetryInitialSeconds, now, actor, value.ID, value.TenantID, expectedVersion)
...
func (r *Repository) RotateSecret(ctx context.Context, tenantID, id string, expectedVersion int64, ciphertext []byte, keyID, actor string) (Subscription, error) {
	now := r.now()
	query := r.db.Rebind(`UPDATE webhook_subscriptions SET secret_ciphertext=?,secret_key_id=?,secret_version=secret_version+1,version=version+1,updated_at=?,updated_by=? WHERE id=? AND tenant_id=? AND version=? AND deleted_at IS NULL`)
	result, err := r.db.ExecContext(ctx, query, ciphertext, keyID, now, actor, id, tenantID, expectedVersion)
	if err := affectedOne(result, err, "rotate webhook subscription secret"); err != nil {
		return Subscription{}, err
	}
	return r.GetSubscription(ctx, tenantID, id)
}

func (r *Repository) DeleteSubscription(ctx context.Context, tenantID, id string, expectedVersion int64, actor string) error {
	now := r.now()
	query := r.db.Rebind(`UPDATE webhook_subscriptions SET status='disabled',deleted_at=?,version=version+1,updated_at=?,updated_by=? WHERE id=? AND tenant_id=? AND version=? AND deleted_at IS NULL`)
	result, err := r.db.ExecContext(ctx, query, now, now, actor, id, tenantID, expectedVersion)
	return affectedOne(result, err, "delete webhook subscription")
}

func (r *Repository) GetSubscription(ctx context.Context, tenantID, id string) (Subscription, error) {
	var value Subscription
	query := r.db.Rebind(`SELECT ` + subscriptionColumns + ` FROM webhook_subscriptions WHERE id=? AND tenant_id=? AND deleted_at IS NULL`)
...
func (r *Repository) ListActiveSubscriptionsTx(ctx context.Context, tx *sqlx.Tx, tenantID string) ([]Subscription, error) {
	items := make([]Subscription, 0)
	query := r.db.Rebind(`SELECT ` + subscriptionColumns + ` FROM webhook_subscriptions WHERE tenant_id=? AND status='active' AND deleted_at IS NULL ORDER BY id`)
	if err := tx.SelectContext(ctx, &items, query, tenantID); err != nil {
...
func (r *Repository) InsertDeliveryTx(ctx context.Context, tx *sqlx.Tx, value Delivery) (bool, error) {
	...
	result, err := tx.ExecContext(ctx, r.db.Rebind(query), value.ID, value.SubscriptionID, value.TenantID, value.EventID, value.EventSubject, value.Payload, value.Status, value.NextAttemptAt, value.CreatedAt, value.UpdatedAt, value.CreatedBy, value.UpdatedBy)
	if err != nil {
...
func (r *Repository) InsertDelivery(ctx context.Context, value Delivery) (Delivery, error) {
	...
	return r.GetDelivery(ctx, value.TenantID, value.ID)
}

func (r *Repository) GetDelivery(ctx context.Context, tenantID, id string) (Delivery, error) {
	var value Delivery
	query := r.db.Rebind(`SELECT ` + deliveryColumns + ` FROM webhook_deliveries WHERE id=? AND tenant_id=?`)
	if err := r.db.GetContext(ctx, &value, query, id, tenantID); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *Repository) ReplayDelivery(ctx context.Context, tenantID, id string, expectedVersion int64, actor string) (Delivery, error) {
	now := r.now()
	query := r.db.Rebind(`UPDATE webhook_deliveries SET status='pending',attempt_count=0,next_attempt_at=?,last_attempt_at=NULL,response_status=0,response_body='',error_message='',delivered_at=NULL,version=version+1,updated_at=?,updated_by=? WHERE id=? AND tenant_id=? AND version=? AND status IN ('succeeded','dead')`)
	result, err := r.db.ExecContext(ctx, query, now, now, actor, id, tenantID, expectedVersion)
	if err := affectedOne(result, err, "replay webhook delivery"); err != nil {
		return Delivery{}, err
	}
	return r.GetDelivery(ctx, tenantID, id)
}
