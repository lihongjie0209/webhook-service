...
func setDefaults(v *viper.Viper) {
	v.SetDefault("app.name", "webhook-service")
	...
	v.SetDefault("database.schema", "webhook")
	...
	v.SetDefault("jwt.issuer", "webhook-service")
	...
	v.SetDefault("auth.audience", "webhook-service")
	...
	v.SetDefault("migration.table", "webhook_schema_migrations")
	...
	v.SetDefault("event_bus.dead_letter_max_data_bytes", 1048576)
	v.SetDefault("webhook.encryption_key", "")
	v.SetDefault("webhook.encryption_key_id", "local-v1")
	v.SetDefault("webhook.allow_http", false)
	v.SetDefault("webhook.allowed_ports", []int{443})
	v.SetDefault("webhook.max_response_bytes", 65536)
	v.SetDefault("webhook.poll_interval", "1s")
	v.SetDefault("webhook.claim_lease", "30s")
	v.SetDefault("webhook.workers", 8)
	v.SetDefault("webhook.batch_size", 50)
	v.SetDefault("webhook.retention", "720h")
	v.SetDefault("webhook.cleanup_interval", "1h")
...
func (c Config) Validate() error {
	...
	if c.EventBus.Enabled && (c.EventBus.ConsumerHandlerTimeout >= c.EventBus.ConsumerAckWait || c.EventBus.ConsumerRetryDelay > c.EventBus.ConsumerMaxRetryDelay) {
		return errors.New("event_bus handler timeout must be shorter than ack wait and retry delay must not exceed maximum")
	}
	if (c.Database.Enabled || c.EventBus.Enabled) && c.Webhook.EncryptionKey == "" {
		return errors.New("enabled webhook persistence or event consumption requires webhook.encryption_key")
	}
	if c.EventBus.Enabled && !c.Database.Enabled {
		return errors.New("webhook event consumption requires database.enabled")
	}
	if c.Webhook.EncryptionKeyID == "" || c.Webhook.MaxResponseBytes <= 0 || c.Webhook.PollInterval <= 0 || c.Webhook.ClaimLease <= 0 || c.Webhook.Workers <= 0 || c.Webhook.BatchSize <= 0 || c.Webhook.Retention <= 0 || c.Webhook.CleanupInterval <= 0 {
		return errors.New("webhook key ID, limits, workers, batch size, and durations must be positive")
	}
	if len(c.Webhook.AllowedPorts) == 0 {
		return errors.New("webhook.allowed_ports must not be empty")
	}
	for _, port := range c.Webhook.AllowedPorts {
		if port < 1 || port > 65535 {
			return fmt.Errorf("webhook.allowed_ports contains invalid port %d", port)
		}
	}
	if c.Webhook.EncryptionKey != "" {
		key, err := base64.StdEncoding.DecodeString(c.Webhook.EncryptionKey)
		if err != nil || len(key) != 32 {
			return errors.New("webhook.encryption_key must be base64 for exactly 32 bytes")
		}
	}
	if c.App.Env == "production" && (c.Webhook.EncryptionKey == "" || c.Webhook.AllowHTTP) {
		return errors.New("production webhook delivery requires an encryption key and HTTPS-only endpoints")
	}
