...
func TestLoad_IdempotencyRouteEnvironmentOverrides(t *testing.T) {
	...
	t.Setenv("APP_IDEMPOTENCY_HTTP_PATHS", "[/api/v1/webhooks/subscriptions/create, /api/v1/webhooks/deliveries/replay]")
	t.Setenv("APP_IDEMPOTENCY_GRPC_METHODS", "[/platform.webhook.v1.WebhookService/CreateSubscription]")
	cfg, err := Load(path)
	if err != nil {
		t.Fatal(err)
	}
	if got := strings.Join(cfg.Idempotency.HTTPPaths, ","); got != "/api/v1/webhooks/subscriptions/create,/api/v1/webhooks/deliveries/replay" {
		t.Fatalf("HTTPPaths = %q", got)
	}
	if got := strings.Join(cfg.Idempotency.GRPCMethods, ","); got != "/platform.webhook.v1.WebhookService/CreateSubscription" {
