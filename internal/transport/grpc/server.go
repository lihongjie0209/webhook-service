...
func NewServer(lc fx.Lifecycle, cfg config.Config, authService *auth.Service, authorizer platformauthz.Authorizer, healthService *apphealth.Service, webhookService *webhookdomain.Service, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, logger *slog.Logger) (*Server, error) {
	...
	webhookv1.RegisterWebhookServiceServer(grpcServer, &webhookServer{service: webhookService})
