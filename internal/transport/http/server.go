...
import (
	...
	docs "github.com/lihongjie0209/webhook-service/docs"
	...
	"github.com/lihongjie0209/webhook-service/internal/ratelimit"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, authorizer platformauthz.Authorizer, limiter *ratelimit.Limiter, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	api.POST("/webhooks/subscriptions/create", handler.CreateSubscription)
	api.POST("/webhooks/subscriptions/update", handler.UpdateSubscription)
	api.POST("/webhooks/subscriptions/get", handler.GetSubscription)
	api.POST("/webhooks/subscriptions/list", handler.ListSubscriptions)
	api.POST("/webhooks/subscriptions/rotate-secret", handler.RotateSubscriptionSecret)
	api.POST("/webhooks/subscriptions/delete", handler.DeleteSubscription)
